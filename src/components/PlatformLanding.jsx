...
const PlatformLanding = () => {
  ...
  useEffect(() => {
    ...
    const recordLandingView = async () => {
      try {
        await supabase.from('qr_scans').insert([{ qr_code_id: qrData.id }])
      } catch (err) {
  ...
  const handlePlatformClick = (platform) => {
    if (!qrData?.id) return
    const resolvedLabel = getDisplayLabel(platform)
    const resolvedUrl = getPlatformUrl(platform)
    supabase
      .from('qr_scans')
      .insert([{ qr_code_id: qrData.id, link_label: resolvedLabel, link_url: resolvedUrl }])
